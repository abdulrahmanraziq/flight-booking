...
class SingleSeats extends Component {
  state = {
    ...
    canChose:
      this.props.preChosen.length === this.props.passengers ? false : true,
  ...
  render() {
    ...
    return (
      <div className="shuttle">
        <h3>{type}</h3>
        <h4>
          {seatClass === "FirstClassSeats"
            ? "First class"
            : seatClass === "BusinessSeats"
            ? "Business class"
            : seatClass === "EconomySeats"
            ? "Economy class"
            : ""}{" "}
          : {this.state.chosenSeats.length}/{passengers}{" "}
        </h4>
        <div className="first-class">
          {this.props.seats
            ? this.subs(this.props.seats).map((r) => (
                <div className="seats-row">
                  {r.map((s) => (
                    <Seat
                      seat={s}
                      parentChosen={this.state.chosenSeats.some(
                        (chosenSeat) => chosenSeat.Seat === s.Seat
                      )}
                      parentChoseSeats={(s, chosen) =>
                        this.handleChoseSeat(s, chosen)
                      }
                      canChose={this.state.canChose}
                    />
                  ))}
                </div>
              ))
